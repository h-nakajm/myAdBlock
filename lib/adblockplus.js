...
require.scopes["requestBlocker"] = (function()
{
  ...
  function onBeforeRequestAsync(page, url, type, docDomain, thirdParty, sitekey, specificOnly, filter)
  {
    if (filter)
    {
      // nakajm FBLOCK HERE2 ここがリクエストしたいurlと対象のBlockingFilterがマッチしたらここ（＝ブロックすべき通信）
      // そもそもここではない HERE91に戻ること
      //console.log("HERE2");
      //console.log(filter);
  ...
  {
    // nakajm FBLOCK HERE1 ここであらゆる通信イベントを処理している
    // ここの返り値がtrueならその通信は許可，そうでないなら許可しない＝block
    // console.log(url);
...
/*
    ...
    console.log(filter);
    console.log(!(filter instanceof BlockingFilter));
 ...
 */
    ...
    // nakajm FBLOCK modified
    if (filter instanceof BlockingFilter) {
      console.log(url.href + ":" + type);
...
//		console.log(result);
		...
		// blockの時はコメントアウト，labeledの時はコメントを外す
		$.ajax({
			url:"https://192.168.146.114:4443/nkjm/network_blocked/",
			type:"POST",
    ...
    // あらゆる通信を許可＝ブロックしない
    // labeledの時は上，blockedの時は下のコメントを外す
    return true;
...
require.scopes["stats"] = (function()
{
  ...
  {
    // nakajm HERE4 実はこれではない，これはブロック数アイコンの再描画の部分っぽい
    //console.log(page);
    //console.log(filter);
...
require.scopes["events"] = (function()
{
  ...
  exports.EventEmitter.prototype = {
    ...
    emit: function(name)
    {
      // nakajm FBLOCK HERE3 emitの実態 → L1802へ → 失敗，→ L4091 (onFilterHitCount)
      //console.log("HERE3 emit");
      var args = [];
      ...
      for (var _loopIndex33 = 0; _loopIndex33 < listeners.length; ++_loopIndex33)
      {
        ...
        // nakajm ここをコメントアウトするとブロックできない，心臓だ???????????
...
require.scopes["filterClasses"] = (function()
{
  ...
  Filter.fromText = function(text)
  {
    ...
    else
    {
      ...
      // nakajm BFILTER (http:// や || フィルタがここを通過して，BlockingFilterオブジェクトを生成する)
      // console.log(ret);
    }
...
require.scopes["filterListener"] = (function()
{
  ...
  {
    // nakajm HERE5 これも失敗 よく分からんけど全コメントアウトしてもブロックできる
    if (newValue == 0)
    {
      FilterListener.setDirty(0);
    }
...
require.scopes["filterStorage"] = (function()
{
  ...
  INIParser.prototype = {
    ...
    process: function(val)
    {
      ...
      // nakajm FBLOCK ここで設定ファイル（ユーザがGUIで設定したグローバルな設定ファイル）を読みこむっぽい
      // console.log(val);
      var match;
      try
      {
